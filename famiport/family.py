...
def doInquiryOrders(cookies={}, ec_order='', text=''):
    ...
    result = requestObj.doRequest(
        url=url, method="POST", headers=headers, json=data)
    return result
...
def getOrderDetail(ec_order_no='', order_no=''):
    ...
    data = {'EC_ORDER_NO': ec_order_no,
            'ORDER_NO': order_no, 'RCV_USER_NAME': None}
    result = requestObj.doRequest(
        url=url, method="POST", headers=headers, json=data)
    return result
...
if __name__ == "__main__":
   ...
   # TODO 店商訂單編號
    ...
    cookies_str = 'ASP.NET_SessionId=' + \
        cookies['ASP.NET_SessionId']+';fmeweb='+cookies['fmeweb']+';'
    # sys.exit()
